...
import { Loader2, GraduationCap, Briefcase, Settings2, Laptop, ArrowRight, Settings, LayoutDashboard, Calendar, Wallet, User, Building2, Mail, Phone } from 'lucide-react';
...
const ClientDashboard = () => {
    ...
    const { company, transactions, isLoading: isLoadingWallet, user } = useWallet();
        ...
        const unsubscribe = onAuthStateChanged(auth, async (user) => {
            ...
            try {
                ...
                if (userDoc.exists()) {
                    ...
                    setEditForm({
                        name: data.name || '',
                        companyName: data.companyName || company?.nome || '',
                        phone: data.phone || ''
                    });
                }
            } catch (error) {
        ...
        <div className="container py-8 space-y-6 animate-fade-in">
            ...
            <div className="flex flex-col md:flex-row md:items-start justify-between gap-4">
                <div>
                    ...
                    <p className="text-muted-foreground text-lg">
                        Gerencie suas locações, carteira e perfil
                    </p>
                </div>

                {/* User Profile Card - Expanded */}
                <Card className="bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900 border-primary/30 min-w-[350px] shadow-xl">
                            ...
                            <div className="flex-1 min-w-0 space-y-2">
                                <div className="flex items-start justify-between gap-2">
                                    <div className="space-y-1 flex-1">
                                        <div className="flex items-center gap-2">
                                            ...
                                            <p className="text-sm font-semibold text-slate-300">Usuário</p>
                                        </div>
                                        <p className="text-base font-bold text-white truncate">{userData?.name || 'Nome não cadastrado'}</p>
                                    </div>
                                    <Sheet open={isEditSheetOpen} onOpenChange={setIsEditSheetOpen}>
                                        <SheetTrigger asChild>
                                            <Button variant="ghost" size="icon" className="shrink-0 hover:bg-white/10 text-slate-300 hover:text-white">
                                ...
                                {userData?.companyName && (
                                    <div className="flex items-center gap-2">
                                        ...
                                        <p className="text-sm text-slate-300 truncate">{userData.companyName}</p>
                                    </div>
                                )}
                                {userData?.phone && (
                                    <div className="flex items-center gap-2">
                                        <Phone className="h-3.5 w-3.5 text-slate-400 shrink-0" />
                                        <p className="text-sm text-slate-300 truncate">{userData.phone}</p>
                                    </div>
                                )}
                            </div>
                        </div>
            ...
            <Tabs defaultValue="dashboard" className="space-y-6">
                <TabsList className="grid w-full grid-cols-3 h-12">
                    ...
                    <TabsTrigger value="locacoes" className="gap-2">
                ...
                <TabsContent value="locacoes" className="space-y-4">
                    <div className="flex items-center gap-2 mb-4">
                        <div className="h-8 w-1 bg-primary"></div>
                        <h2 className="text-2xl font-bold uppercase tracking-tight">Todas as Minhas Locações</h2>
                    </div>
                    <RentalsList />
                </TabsContent>
                ...
                <TabsContent value="wallet" className="space-y-6">
                    {company ? (
                        ...
                        <Card className="p-8 text-center">
                            ...
                            <h3 className="text-xl font-bold mb-2">Ative sua EXS Wallet</h3>
                            <p className="text-muted-foreground mb-6">
                                Cadastre sua empresa para começar a acumular cashback.
                            </p>
                        </Card>
                    )}
                </TabsContent>
